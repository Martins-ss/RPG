...
import { Skull, Swords, Heart, Gift, BookOpen, Zap } from 'lucide-react';
    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="space-y-4">
        {BOSSES.map((boss, idx) => {
          ...
          if (!phase) { console.error(`[BossesPanel] Fase não encontrada para boss: ${boss.name}`); return null; }
          const currentHealth = bossHealths[boss.name] ?? boss.maxHealth;
          const isExpanded = selectedBoss === idx;
          const isDead = currentHealth === 0;

          return (
            <div key={boss.name} className="card-dark rounded-xl overflow-hidden"
              style={{ borderColor: isDead ? 'rgba(34,197,94,0.3)' : `${phase.color}20` }}>

              <button
                onClick={() => { console.log(`[BossesPanel] Toggling boss ${idx}`); setSelectedBoss(isExpanded ? null : idx); }}
                className="w-full p-4 text-left"
              >
                <div className="flex items-center gap-3">
                  <div className={`w-16 h-16 rounded-xl flex items-center justify-center text-3xl shrink-0 ${isDead ? 'opacity-40 grayscale' : ''}`}
                    style={{ background: `${phase.color}15`, border: `1px solid ${phase.color}30` }}>
                    {boss.emoji}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className={`text-base font-bold ${isDead ? 'line-through text-gray-600' : ''}`}
                        style={{ color: isDead ? undefined : phase.color, fontFamily: 'Cinzel, serif' }}>
                        {boss.name}
                      </h3>
                      {isDead && <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-900/30 text-green-400">DERROTADO</span>}
                    </div>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 space-y-4 animate-slideIn">
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); console.log(`[BossesPanel] Abrindo história do boss ${idx}`); setShowStory(idx); }}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      ...
                      <button onClick={e => { e.stopPropagation(); bossAttackAll(boss.name); }}
                  ...
                  <div className="p-3 rounded-lg" style={{ background: getRarityBg(boss.reward.rarity), border: `1px solid ${getRarityColor(boss.reward.rarity)}30` }}>
                    <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="inline-block mt-1 text-[10px] px-1.5 py-0.5 rounded" style={{
      ...
      <Modal isOpen={showStory !== null} onClose={() => setShowStory(null)} title={showStory !== null ? `📖 ${BOSSES[showStory].name}` : ''}>
        {showStory !== null && (
          <div className="space-y-4">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500 mt-1">{PHASES.find(p => p.name === BOSSES[showStory].phase)?.emoji} {BOSSES[showStory].phase}</p>
            </div>
